@if (setting('payment_multisafepay_status') == 1)
    <li class="list-group-item">
        <input class="magic-radio js_payment_method" type="radio" name="payment_method" id="payment_multisafepay"
               @if ($selecting == MULTISAFEPAY_PAYMENT_METHOD_NAME) checked @endif
               value="multisafepay" data-bs-toggle="collapse" data-bs-target=".payment_multisafepay_wrap" data-toggle="collapse" data-target=".payment_multisafepay_wrap" data-parent=".list_payment_method">
        <label for="payment_multisafepay" class="text-start">{{ setting('payment_multisafepay_name', trans('plugins/payment::payment.payment_via_multisafepay')) }}</label>
